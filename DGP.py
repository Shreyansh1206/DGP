...
def projection(args, sess, cloth_dir, parse_residual_np, step=300):
    ...
    rough_alignment = cv2.cvtColor(
        cv2.resize(
            cv2.imread(rough_alignment_path), (512, 512), interpolation=cv2.INTER_CUBIC
        ),
        cv2.COLOR_BGR2RGB,
    )
    ...
    args.x_np = (
        np.transpose(rough_alignment[np.newaxis,], [0, 3, 1, 2]) / 127.5 - 1
    ).astype("float32")
    latent_code = sess.run(
        args.latent_code_placeholder, {args.rough_align_placeholder: args.x_np}
    )

    setter_w = tf.assign(args.wp, latent_code)
    setter_n = [
        tf.assign(var, npvar) for var, npvar in zip(args.noise_vars, args.noise_np)
    ]

    _ = sess.run([setter_w] + setter_n, {args.x: args.x_np})
    encoder_result = sess.run(
        args.generate_img_placeholder, {args.rough_align_placeholder: args.x_np}
    )
...
def semantic_search(
    args, sess, cloth_dir, model_dir, outputs_projection, mask_inner, mask_arm, step=300
):
    set_semantic_and_pattern_loss(
        args, cloth_dir, model_dir, outputs_projection, mask_inner, mask_arm
    )
...
def pattern_search(
    ...
    mask_train_op_n = optimizer_mask_n.minimize(
        args.loss_mask, var_list=args.noise_vars
    )
    tflib.init_uninitialized_vars()
    setter_n = [
        tf.assign(var, npvar) for var, npvar in zip(args.noise_vars, args.noise_np)
...
def set_projection_loss(args):
    ...
    # Pixel loss
    loss_pix = tf.reduce_mean(
        tf.square((args.x - args.x_rec) * args.parse_residual), axis=[1, 2, 3]
    )

    args.projection_loss = (
        loss_pix + 5e-5 * loss_feat + 1 * loss_adv + loss_attr * 0.00005
    )


def set_semantic_and_pattern_loss(
    args, cloth_dir, model_dir, outputs_projection, mask_inner, mask_arm
):
    # Define semantic loss and pattern loss
    loss_inter = tf.reduce_mean(
        tf.square(args.x - args.x_rec) * mask_inner, axis=[1, 2, 3]
    )
    loss_arm = tf.reduce_mean(
        tf.square(outputs_projection[1] - args.x_rec) * mask_arm, axis=[1, 2, 3]
    )
    loss_outer = tf.reduce_mean(
        tf.square(outputs_projection[1] - args.x_rec)
        * (1 - mask_arm)
        * (1 - mask_inner),
        axis=[1, 2, 3],
    )
