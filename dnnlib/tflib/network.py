...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        with (
            tfutil.absolute_variable_scope(self.scope, reuse=False),
            tfutil.absolute_name_scope(self.scope),
    ...
    def get_output_for(
        ...
        with (
            tfutil.absolute_variable_scope(self.scope, reuse=True),
            tf.name_scope(self.name),
        ):
            assert tf.get_variable_scope().name == self.scope
    ...
    def run(
        ...
        **dynamic_kwargs,
        ...
        if key not in self._run_cache:
            with (
                tfutil.absolute_name_scope(self.scope + "/_Run"),
