...
class Optimizer:
    ...
    def _get_device(self, device_name: str):
        ...
        with (
            tfutil.absolute_name_scope(self.scope + "/Devices"),
    ...
    def apply_updates(self, allow_no_op: bool = False) -> tf.Operation:
        ...
        for device_idx, device in enumerate(self._devices.values()):
            with (
                tfutil.absolute_name_scope(self.scope + "/Clean%d" % device_idx),
                tf.device(device.name),
            ):
        ...
        if len(self._devices) > 1:
            with tfutil.absolute_name_scope(self.scope + "/Broadcast"), tf.device(None):
                ...
                ):  # Windows => NCCL ops are not available.
        ...
        for device_idx, device in enumerate(self._devices.values()):
            with (
                tfutil.absolute_name_scope(self.scope + "/Apply%d" % device_idx),
                tf.device(device.name),
            ):
    ...
    def _broadcast_fallback(self):
        ...
        global \
            ...
            _collective_ops_group_key, \
            _collective_ops_instance_key
        if all(
            x.shape.num_elements() == 0
            for device in self._devices.values()
            for x in device.grad_clean.values()
        ):
            return
        ...
        for device in self._devices.values():
            with tf.device(device.name):
                combo = [
                    tf.reshape(x, [x.shape.num_elements()])
                    for x in device.grad_clean.values()
                ]
                combo = tf.concat(combo, axis=0)
                combo = collective_ops.all_reduce(
                    combo,
                    merge_op="Add",
                    final_op="Id",
                    group_size=len(self._devices),
                    group_key=_collective_ops_group_key,
                    instance_key=_collective_ops_instance_key,
                )
                cur_ofs = 0
                for var, grad_old in device.grad_clean.items():
                    grad_new = tf.reshape(
                        combo[cur_ofs : cur_ofs + grad_old.shape.num_elements()],
                        grad_old.shape,
                    )
